...
-- Query 6: Retrieve customer details for John Doe
SELECT * FROM Customers WHERE FirstName = 'John' AND LastName = 'Doe';

-- Query 7: Retrieve orders made by John Doe
SELECT * FROM Orders WHERE CustomerID = (SELECT CustomerID FROM Customers WHERE FirstName = 'John' AND LastName = 'Doe');

-- Query 8: Retrieve payments made by John Doe
SELECT * FROM Payments WHERE OrderID IN (SELECT OrderID FROM Orders WHERE CustomerID = (SELECT CustomerID FROM Customers WHERE FirstName = 'John' AND LastName = 'Doe'));
...
-- Nitesh Section End
-- NAveed Section
-- Query 36: Retrieve payments made on June 1, 2024
...
WHERE CustomerID IN (
    ...
    WHERE o.PackageID = 3 AND p.Amount > 10
);
...
FROM Orders 
WHERE CustomerID IN (
    SELECT CustomerID FROM Customers WHERE Phone LIKE '123%'
);
...
-- Query 42: Retrieve orders made by customers with addresses containing 'Main St'
SELECT * 
FROM Orders 
WHERE CustomerID IN (
    SELECT CustomerID FROM Customers WHERE Address LIKE '%Main St%'
);
...
SELECT * FROM Payments WHERE PaymentDate BETWEEN '2024-06-01' AND '2024-06-15';
